...
import {useActions, useAppDispatch, useThunks} from "../../../common/helpers/reduxHook";
...
export const Header = () => {
    ...
    let getDate = (string: any) => new Date(0, 0,0, string.split(':')[0], string.split(':')[1]); //получение даты из строки (подставляются часы и минуты
    ...
    useEffect(() => {
        ...
        if (startTime && startTime?.replace(':', '')) {
            const interval = setInterval(() => {
                ...
                let different = (Number(getDate(currentTime)) - Number(getDate(firstTIme)));


                let hours = Math.floor((different % 86400000) / 3600000);
                let minutes = Math.round(((different % 86400000) % 3600000) / 60000);
                let result = (hours > 9 ? hours : `0${hours}`) + ':' + (minutes > 9 ? minutes : `0${minutes}`);
            ...
            return () => clearInterval(interval)
        } else if (startTimeAuto && startTimeAuto?.replace(':', '')) {
            const interval = setInterval(() => {
    ...
    return (
        <header className={s.Header}>
            ...
            <Modal isVisible={isOpenModal}
                   ...
                   content={<p>Вы уверены что хотите выйти в стартовый экран без сохранения чек-листа?</p>}
                   footer={<Button classname={s.homeBtn} onClick={onClickCloseBtnHandler}>Выйти без сохранения</Button>}/>
        </header>
    );
};
