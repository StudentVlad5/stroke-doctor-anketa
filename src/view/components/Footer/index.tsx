...
export const Footer = () => {
    ...
    return (
        <footer className={s.Footer}>
            { location.pathname !== "/1" ? <Button onClick={onClickPrevBtnHandler} classname={s.prevBtn}>Назад</Button> : <div style={{width: "100%"}}></div> }
            <div className={s.page}>{location.pathname[1]}/5</div>
            { location.pathname !== '/5' ? <Button onClick={onClickNextBtnHandler} classname={s.nextBtn}>Дальше</Button> : <Button classname={s.endBtn} onClick={onClickFinishBtnHandler}>Пациент передан</Button>}
            <Modal isVisible={isOpenModal} notClose={true}
                   content={<p>Чек-лист успешно заполнен и отправлен</p>}
                   footer={<button className={s.successBtn} onClick={onClickSuccessButton}>Ок</button>}/>
        </footer>
    );
};
